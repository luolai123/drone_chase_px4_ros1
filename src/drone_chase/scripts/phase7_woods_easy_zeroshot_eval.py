...
DEFAULT_OUTPUT_DIR = "/home/whk/vf_ws/outputs/phase7/woods_easy_zeroshot_from_final_policy"
DEFAULT_CONFIG = (
...
def verify_registry_policy(model_path, vecnormalize_path, manifest_path):
    if os.path.abspath(model_path) != os.path.abspath(REGISTERED_MODEL):
        raise RuntimeError("Phase 7.3A must load registered policy only; got model={}".format(model_path))
    if os.path.abspath(vecnormalize_path) != os.path.abspath(REGISTERED_VECNORMALIZE):
        raise RuntimeError(
            "Phase 7.3A must load registered VecNormalize only; got vecnormalize={}".format(vecnormalize_path)
        )
...
def env_kwargs_from_config(config, seed, world):
    ...
    env_cfg.setdefault("woods_easy_num_trunks", 8)
    env_cfg.setdefault("woods_easy_num_branches", 15)
    env_cfg.setdefault("woods_easy_num_fallen", 3)
    env_cfg.setdefault("woods_easy_area_x_min", 1.5)
    env_cfg.setdefault("woods_easy_area_x_max", 6.5)
    env_cfg.setdefault("woods_easy_area_y_min", -3.0)
    env_cfg.setdefault("woods_easy_area_y_max", 3.0)
    env_cfg.setdefault("woods_easy_uav_clearance", 1.5)
    env_cfg.setdefault("woods_easy_target_clearance", 0.8)
...
def analyze_failures(failures, summary):
    ...
    if summary["mean_target_visible_ratio"] < 0.8:
        lines.append("perception: low mean target visibility suggests woods occlusion or detector limits")
    if summary["mean_safety_filtered_count"] > 20.0:
        lines.append("safety_filter: frequent filtering may prevent approach through woods")
...
def build_diagnostic_answers(summary):
    ...
    return [
        ...
        "policy 疑似偏直线追球、难处理树木遮挡：{}".format("是" if straight_line_risk else "否"),
        "red_ball spawn 与 trees/obstacles 可能形成不可见或不可达布局：{}".format(
            "是" if spawn_layout_issue else "否"
        ),
        "depth ROI 对 woods 场景可能过于敏感：{}".format("是" if roi_sensitive else "否"),
        "是否需要 woods_easy DAgger demos：{}".format("是，建议先采集诊断/示范再训练" if adaptation_needed else "暂不需要"),
        "是否需要 woods_easy BC adaptation：{}".format("是，若后续允许训练应先走 BC/DAgger adaptation" if adaptation_needed else "暂不需要"),
    ]
...
def write_report(path, summary):
    ...
    passed_text = "是" if summary["phase7_3a_passed"] else "否"
    phase7_3b_text = "是" if summary["phase7_3b_allowed"] else "否"
    woods_claim_text = "是" if summary["woods_claim_allowed"] else "否"
    lines = [
        "# Phase 7.3A Woods Easy Zero-Shot Validation",
        "",
        ...
        "22. 是否通过 woods_easy zero-shot gate：{}".format(passed_text),
        "23. 是否允许进入 Phase 7.3B：{}".format(phase7_3b_text),
        "24. 是否允许声称 woods 已通过：{}".format(woods_claim_text),
        "",
        "Gate detail:",
    ...
    lines.extend([
        ...
        "- This is zero-shot evaluation of the frozen world0/world1 sparse policy; no woods training or fine-tuning was performed.",
        "- Passing Phase 7.3A only permits Phase 7.3B woods_easy robustness validation; it does not mean woods is complete.",
    ])
    with open(path, "w") as handle:
        handle.write("\n".join(lines) + "\n")
...
def write_reproduce_commands(path, args):
    content = """# Phase 7.3A Woods Easy Zero-Shot Reproduce Commands
...
def build_arg_parser():
    parser = argparse.ArgumentParser(description="Phase 7.3A woods_easy zero-shot eval.")
    ...
    parser.add_argument("--world", default="woods_easy", choices=["woods_easy"])
...
def main():
    ...
    rollout_csv = os.path.join(args.output_dir, "woods_easy_zeroshot_rollouts.csv")
    episode_csv = os.path.join(args.output_dir, "woods_easy_zeroshot_episodes.csv")
    failure_csv = os.path.join(args.output_dir, "failure_cases.csv")
    safety_csv = os.path.join(args.output_dir, "safety_events.csv")
    summary_json = os.path.join(args.output_dir, "woods_easy_zeroshot_summary.json")
    report_md = os.path.join(args.output_dir, "phase7_3a_report.md")
    ...
    woods_cfg = {
        "num_trunks": env_cfg.get("woods_easy_num_trunks"),
        "num_branches": env_cfg.get("woods_easy_num_branches"),
        "num_fallen": env_cfg.get("woods_easy_num_fallen"),
        "area_x_min": env_cfg.get("woods_easy_area_x_min"),
        "area_x_max": env_cfg.get("woods_easy_area_x_max"),
        "area_y_min": env_cfg.get("woods_easy_area_y_min"),
        "area_y_max": env_cfg.get("woods_easy_area_y_max"),
        "uav_clearance": env_cfg.get("woods_easy_uav_clearance"),
        "target_clearance": env_cfg.get("woods_easy_target_clearance"),
    }
    summary = {
        "phase": "7.3A",
        ...
        "phase7_3a_passed": bool(phase_passed),
        "phase7_3b_allowed": bool(phase_passed),
    ...
    print("phase7_3a_passed={}".format(summary["phase7_3a_passed"]))
...
if __name__ == "__main__":
    try:
        ...
        print("phase7_woods_easy_zeroshot_eval failed: {}".format(exc), file=sys.stderr)
        sys.exit(1)
