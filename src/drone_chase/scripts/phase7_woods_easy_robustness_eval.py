...
from phase7_woods_easy_zeroshot_eval import (  # noqa: E402
    CommandMonitor,
    MANIFEST_PATH,
    REGISTERED_MODEL,
...
DEFAULT_OUTPUT_DIR = "/home/whk/vf_ws/outputs/phase7/woods_easy_robustness_from_final_policy"
DEFAULT_CONFIG = (
...
GROUP_SPECS = [
    {
        "group": "A",
        "name": "woods_easy_standard_recheck",
        "episodes": 30,
        "num_trunks": 8,
        "num_branches": 15,
        "num_fallen": 3,
    ...
    },
    {
        ...
        "target_clearance": 0.8,
        "target_distance": 4.0,
        "target_lateral_cycle": [-0.35, 0.0, 0.35],
    },
    {
        "group": "C",
        "name": "woods_easy_safety_stress",
        "episodes": 20,
        "num_trunks": 10,
        "num_branches": 20,
        "num_fallen": 4,
    ...
    env.config.update(
        {
            ...
            "seed": int(seed),
            "woods_easy_num_trunks": int(spec["num_trunks"]),
            "woods_easy_num_branches": int(spec["num_branches"]),
            "woods_easy_num_fallen": int(spec["num_fallen"]),
            "woods_easy_area_x_min": float(spec["area_x_min"]),
            "woods_easy_area_x_max": float(spec["area_x_max"]),
            "woods_easy_area_y_min": float(spec["area_y_min"]),
            "woods_easy_area_y_max": float(spec["area_y_max"]),
            "woods_easy_uav_clearance": float(spec["uav_clearance"]),
            "woods_easy_target_clearance": float(spec["target_clearance"]),
...
def build_summary(args, model_sha, vec_sha, episode_rows, rollout_rows, reset_rows, safety_events):
    ...
    return {
        ...
        "phase7_3b_passed": phase_passed,
        "phase7_3c_allowed": phase_passed,
        "woods_easy_claim_allowed": phase_passed,
        "woods_all_claim_allowed": False,
...
def write_report(path, summary):
    ...
    lines = [
        "# Phase 7.3B Woods Easy Robustness / Stress Validation",
        "",
        ...
        "17. mean episode length：{:.4f}".format(summary["mean_episode_length"]),
        "18. RAW_TIMEOUT count：{}".format(summary["raw_timeout_count"]),
        "19. emergency_count：{}".format(summary["emergency_count"]),
        "20. depth_stop_count：{}".format(summary["depth_stop_count"]),
        "21. front_q05_min_mean：{:.4f} m".format(summary["front_q05_min_mean"]),
        "22. obstacle_danger_steps_mean：{:.4f}".format(summary["obstacle_danger_steps_mean"]),
        "23. OFFBOARD drop count：{}".format(summary["offboard_drop_count"]),
        "24. reset pollution detected：{}".format("是" if summary["reset_pollution_detected"] else "否"),
        "25. main failure modes：{}".format(summary["main_failure_modes"]),
        "26. 是否通过 Phase 7.3B：{}".format("是" if summary["phase7_3b_passed"] else "否"),
        "27. 是否允许进入 Phase 7.3C：{}".format("是" if summary["phase7_3c_allowed"] else "否"),
        "28. 是否允许声称 woods_easy 已通过：{}".format(
            "是" if summary["woods_easy_claim_allowed"] else "否"
        ),
        "29. 是否允许声称 woods 全部已通过：{}".format(
            "是" if summary["woods_all_claim_allowed"] else "否"
        ),
        "",
        "Diagnostics:" if summary["phase7_3b_passed"] else "Failure diagnosis:",
    ]
...
def write_reproduce_commands(path, args):
    content = """# Phase 7.3B Woods Easy Robustness Reproduce Commands
...
def output_paths(args):
    return {
        "rollout_csv": os.path.join(args.output_dir, "woods_easy_robustness_rollouts.csv"),
        "episode_csv": os.path.join(args.output_dir, "woods_easy_robustness_episodes.csv"),
        "safety_csv": os.path.join(args.output_dir, "safety_events.csv"),
        "summary_json": os.path.join(args.output_dir, "woods_easy_robustness_summary.json"),
        "report_md": os.path.join(args.output_dir, "phase7_3b_report.md"),
...
def summarize_existing_outputs(args):
    ...
    print("phase7_3b_passed={}".format(summary["phase7_3b_passed"]))


def build_arg_parser():
    parser = argparse.ArgumentParser(description="Phase 7.3B woods_easy robustness/stress eval.")
    ...
    parser.add_argument("--seed-base", type=int, default=73100)
...
if __name__ == "__main__":
    try:
        ...
        print("phase7_woods_easy_robustness_eval failed: {}".format(exc), file=sys.stderr)
        sys.exit(1)
